...
try {
    ...
    if ($stmt->execute()) {
            ...
            if ($itemType === 'product') {
                ...
                while ($bundleProduct = $bundleProductsStmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if ($productStock >= ($bundleProductQuantity * $quantity)) {
                        ...
                        throw new Exception('Not enough stock for product in bundle');
                    }
                }
                ...
                while ($eventProduct = $eventProductsStmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if ($productStock >= ($eventProductQuantity * $quantity)) {
                        ...
                        throw new Exception('Not enough stock for product in event');
                    }
                }
            }
        }
        ...
        if ($data) {
            ...
            header("Location: ./profile/success-order.php");
            exit();
        }
    } else {
        throw new Exception('Failed to process the order');
    }
